...
for (var i = 0; i < a.length; i++) {
	for (var j = 0; j < a[i].length - 3; j++) {
		...
		var prod = a[i][j] * a[i][j+1] * a[i][j+ 2] * a[i][j+ 3];

		// console.log(a[i][j] + ' ' + a[i][j+1] + ' ' + a[i][j+ 2] + ' ' + a[i][j+ 3])

		if (prod > max) {
			console.log('Horizontal Maximum');
			max = prod;
		}
	}
};
...
for (var i = 0; i < a.length - 3; i++) {
	for (var j = 0; j < a[i].length; j++) {
		...
		var prod = a[i][j] * a[i+1][j] * a[i+2][j] * a[i+3][j];

		// console.log(a[i][j] + ' ' + a[i+1][j] + ' ' + a[i+2][j] + ' ' + a[i+3][j])

		if (prod > max) {
			console.log('Vertical Maximum');
			max = prod;
		}
	}
};

// Check for a diagonal maximum. (down & right)
for (var i = 0; i < a.length - 3; i++) {
	for (var j = 0; j < a[i].length - 3; j++) {

		var prod = a[i][j] * a[i+1][j+1] * a[i+2][j+2] * a[i+3][j+3];

		// console.log(a[i][j] + ' ' + a[i+1][j+1] + ' ' + a[i+2][j+2] + ' ' + a[i+3][j+3]);

		if (prod > max) {
			console.log('Down and Right Max');
			max = prod;
		}
	}
};
...
// Check for a diagonal maximum. (down & left)
